...
export async function inspectPreview(baseUrl, options = {}) {
  ...
  for (const path of previewPaths) {
    try {
      ...
      const robotsMetaMatch = text.match(/<meta[^>]+name=["']robots["'][^>]+content=["']([^"']+)/i) ?? text.match(/<meta[^>]+content=["']([^"']+)["'][^>]+name=["']robots["']/i);
      const robotsMeta = !htmlPaths.has(path) || Boolean(robotsMetaMatch?.[1]?.toLowerCase().includes("noindex") && robotsMetaMatch[1].toLowerCase().includes("nofollow"));
