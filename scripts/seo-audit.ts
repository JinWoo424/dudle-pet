...
async function main(){
 ...
 const matrix:MatrixRow[]=rows.map(row=>{
  ...
  const currentH1=pageType==="FUNERAL_REGION"?`${label} 반려동물 장례시설`:primary;
  return {category:pageType,province,city,district,canonical_url:String(row.canonical_url),primary_keyword:primary,secondary_keyword_1:`${primary} ${config.secondary[0]}`,secondary_keyword_2:`${primary} ${config.secondary[1]}`,open_count:count,coordinate_count:coordinates,data_quality:quality>=75?"HIGH":quality>=55?"MEDIUM":"LOW",seo_status:String(row.seo_status),current_title:currentTitle,proposed_title:proposed,current_h1:currentH1,proposed_h1:primary,priority:priority(pageType,label,String(row.seo_status),count,quality),description:description(pageType,label,count,coordinates,lastmod),lastmod};
 });
