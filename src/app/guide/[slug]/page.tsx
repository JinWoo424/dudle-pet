...
export async function generateMetadata({ params }: Props): Promise<Metadata> { const { slug } = await params; const guide = guideEntries.find((item) => item.slug === slug); const canonical=`/guide/${slug}`; return guide ? { title: guide.title, description: guide.summary, alternates:{canonical}, ...shareMetadata(guide.title,guide.summary,canonical) } : {}; }
