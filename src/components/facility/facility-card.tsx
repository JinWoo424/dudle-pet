...
export function FacilityCard({facility:f,compact=false}:{facility:FacilityView;compact?:boolean}){
 ...
 return <article className={`card facility-card${compact?" compact":""}`}><div className="facility-card-top"><div><span className="status-open">{statusLabels[f.businessStatus]}</span><h2>{path?<Link href={path} prefetch={false}>{f.name}</Link>:f.name}</h2></div>{f.distanceMeters!=null&&<strong className="distance">{formatDistance(f.distanceMeters)}<small> 직선거리</small></strong>}</div>
 ...
 <div className="facility-actions">{f.phone?<a href={`tel:${f.phone}`}><Phone size={16}/>전화</a>:<span>전화 미확인</span>}{f.roadAddress&&<a href={directionsUrl(f)} target="_blank" rel="noreferrer"><Navigation size={16}/>길찾기</a>}{path&&<NavigationLink className="detail-link" href={path} prefetch={false}>상세보기</NavigationLink>}</div></article>;
}
