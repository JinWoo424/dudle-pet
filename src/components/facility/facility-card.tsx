...
export function FacilityCard({facility:f,compact=false}:{facility:FacilityView;compact?:boolean}){
 ...
 <div className="facility-actions">{f.phone?<a href={`tel:${f.phone}`}><Phone size={16}/>전화</a>:<span>전화 미확인</span>}{f.roadAddress&&<a href={directionsUrl(f)} target="_blank" rel="noreferrer"><Navigation size={16}/>길찾기</a>}{path&&<Link className="detail-link" href={path} prefetch={false}>상세보기</Link>}</div></article>;
}
