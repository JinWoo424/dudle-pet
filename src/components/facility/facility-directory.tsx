...
export async function FacilityDirectory({type,segments,query={}}:{type:FacilityKind;segments:string[];query?:Record<string,string|string[]|undefined>}){
 ...
 {result!.total>30&&<nav className="chip-list" aria-label="페이지">{result!.page>1&&<Link className="chip-link" href={`?page=${result!.page-1}&sort=${query.sort==="name"?"name":"quality"}`} prefetch={false}>이전</Link>}{pageLinks.map(page=><Link aria-current={page===result!.page?"page":undefined} className="chip-link" href={`?page=${page}&sort=${query.sort==="name"?"name":"quality"}`} key={page} prefetch={false}>{page}</Link>)}{result!.page*30<result!.total&&<Link className="chip-link" href={`?page=${result!.page+1}&sort=${query.sort==="name"?"name":"quality"}`} prefetch={false}>다음</Link>}</nav>}
